...
// aucun secret de production n'est requis ni écrit.
//
...
// extraire libnspr4, libnss3 et libasound2 dans un dossier et les fournir par
// LD_LIBRARY_PATH.
...
async function capturerApplication(navigateur, session) {
  for (const theme of THEMES) {
    for (const format of FORMATS) {
      ...
      if (!reponse?.ok()) {
        throw new Error(`l'application a répondu ${reponse?.status()} — session ou serveur en cause`)
      }
      ...
      const chemin = `${sortie}/${etiquette}-${theme.nom}-${format.nom}.png`
...
async function capturerDesign(navigateur) {
  ...
  for (const theme of THEMES) {
    ...
    for (const format of FORMATS) {
      ...
      const chemin = `${sortie}/design-${theme.nom}-${format.nom}.png`
